...
- __간단한 해결책__<br/>
처음 루프에서 함수를 즉시실행하면 된다. 함수가 즉시 실행돼서(함수) arr에 담기는 값은 function이 아니라 return값 i 가 된다. console.log(arr\[index]()); 이 루프 또 한 function 이 아닌 int이므로 생성자 (); 를 빼주면 0,1,2,3,4를 볼수 있다.
```
var arr = []
for(var i = 0; i < 5; i++){
  arr[i] = function(){
