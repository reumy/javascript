...
### 재사용성, 유지보수 용이, 가독성
- 반복문은 그 자리에서 일정한 행위를 여러번 반복하는데 의의가 있고<br/>함수는 반복적인 로직이 여러가지 맥락에서 반복돼서 사용될 때 의의가 있음
<br/>즉, 반복문은 그 자리에서만 반복적으로 실행되기 때문에 여러가지 맥락에서 반복문을 사용해야할때 함수를 이용하면 유용
...
```
> get_argument(1), get_argument(2)는 각각 get_argument로 1과 2이라는 값을 전달하겠다는 의미로 정의된 매개변수 (arg) 구문에 의해서 변수 arg의 값으로 숫자 1과 2가 함수 안으로 전달됨<br/>단, 이 변수 arg는 함수 get_argument 안에서만 유효함
